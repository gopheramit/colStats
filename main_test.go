...
func TestRun(t *testing.T) {
	testCases := []struct {
		...
		{name: "RunAvg1File", col: 3, op: "avg", exp: "233.84\n", files: []string{"./testdata/example.csv", "testdata/example2.csv"}, expErr: nil},
		{name: "RunAvgMultiFiles", col: 3, op: "avg", exp: "233.84\n", files: []string{"./testdata/example.csv", "./testdata/example2.csv"}, expErr: nil},

		{name: "RunFailRead", col: 2, op: "avg", exp: "", files: []string{"./testdata/example.csv", "./testdata/fakefilecls.csv"}, expErr: os.ErrNotExist},
		{name: "RunFailColumn", col: 0, op: "avg", exp: "", files: []string{"./testdata/examaple.csv"}, expErr: ErrInvalidColumn},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("Unexpectd error:%q", err)
			}
